...
import gradio as gr
import numpy as np
from gradio.components import Textbox, Video, Slider, Checkbox
...
def setup_environment():
    os.system("git clone -b dev https://github.com/camenduru/Open-Sora-Plan-v1.0.0-hf")
    os.chdir("Open-Sora-Plan-v1.0.0-hf")
    os.system("pip install -q diffusers==0.24.0 gradio==3.50.2 einops==0.7.0 omegaconf==2.1.1 pytorch-lightning==1.4.2 torchmetrics==0.6.0 torchtext==0.6 accelerate==0.28.0")
...
def generate_img(prompt, sample_steps, scale, seed=0, randomize_seed=False, force_images=False):
    ...
    height, width = map(int, args.version.split('x')[1:3])
...
if __name__ == '__main__':
    ...
    videogen_pipeline = VideoGenPipeline(vae=vae, text_encoder=text_encoder, tokenizer=tokenizer, scheduler=scheduler, transformer=transformer_model).to(device=device)

    demo = gr.Interface(
        fn=generate_img,
        inputs=[
            Textbox(label="", placeholder="Please enter your prompt."),
            Slider(label='Sample Steps', minimum=1, maximum=500, value=50, step=10),
            Slider(label='Guidance Scale', minimum=0.1, maximum=30.0, value=10.0, step=0.1),
            Slider(label="Seed", minimum=0, maximum=203279, step=1, value=0),
            Checkbox(label="Randomize seed", value=True),
            Checkbox(label="Generate image (1 frame video)", value=False)
        ],
        outputs=[
